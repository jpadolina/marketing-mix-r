...
#Final test: Aggregate spend variables
advOnline <- multdata$ADV_online
advOffline <- multdata$ADV_Offline
SqOn <- sqrt(advOnline)
SqOff <- sqrt(advOffline)
lnOn <- log(advOnline)
lnOff <- log(advOffline)

#Replace -Inf values in lnOff

lnOff <- sapply(lnOff, replace_inf)

#Square root model with lag and intercept
aggMod <- lm(Sales ~ Stm1+SqOn+SqOff)
summary(aggMod)
AIC(aggMod) #higher than bestmodel
BIC(aggMod) #higher than bestmodel

aggMod2 <- lm(Sales ~ Stm1+lnOn+lnOff)
summary(aggMod2)
AIC(aggMod2) #improved but higher than bestmodel
BIC(aggMod2) #improved but higher than bestmodel
